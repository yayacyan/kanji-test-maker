...
- **2026-09-23**：答えが見えてしまう／マスが小さいときに印刷前の確認画面を追加。練習マス・なぞり書きのマスの高さをそろえた。送りがなの説明を分かりやすくした。文中の□の読みがな既定をオフに（送りがなは維持）。学年・組／日付の既定をオンに。用紙サイズのおすすめを実測ベースに改善し、極端に問題数が多いときのレイアウト崩れを修正。
